...
import { getProductionOrders, getJobs, updateProductionPlan, getJobRolesConfig } from '../services/dataService';
...
const IslerUretimPlani = () => {
  ...
  const handleDropOnCalendar = (e, date) => {
    ...
    const assemblyDate = getJobAssemblyDate(draggedOrder.jobId);
    if (assemblyDate) {
  ...
  const handleDropOnUnplanned = (e) => {
    e.preventDefault();
    if (!draggedOrder || !draggedOrder.plannedDate) return;
          ...
          <div className="card" style={{ padding: 20 }}>
            ...
            <div style={{ 
              ...
              {calendarDays.map((dayInfo, idx) => {
                ...
                const cellHeight = viewMode === 'weekly' ? 300 : 140;
                const maxVisibleOrders = viewMode === 'weekly' ? 8 : 3;
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {dayOrders.slice(0, maxVisibleOrders).map(order => (
                        <div
                          ...
                          style={{
                            padding: viewMode === 'weekly' ? '8px 10px' : '5px 8px',
                        ...
                        >
                          {/* Müşteri Adı + İş Kolu Kısaltması */}
                          <span>{order.customerName || 'Müşteri'}</span>
                          {order.roleShort && (
                            <span style={{ 
                              marginLeft: 6, 
