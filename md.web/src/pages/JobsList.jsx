...
const JobStepper = ({ job, customers = [], onUpdated }) => {
  ...
  const loadProductionData = async () => {
    ...
    return { ordersData, rolesData, suppliersData, glassData, combData, teamsData, assemblyData };
  };
    ...
    <div className="grid grid-1" style={{ gap: 16 }}>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      await updateProductionStatus(job.id, { status: 'MONTAJA_HAZIR' });
                      await refreshJob();
                    } catch (err) {
                      alert(toMessage(err));
                    } finally {
      ...
      {isStageSelected('assembly') && (
        <div className="card">
          ...
          {/* Montaj Görevleri Özeti */}
          {assemblyTasksData.roles?.length > 0 && (
            <div style={{ 
              padding: '1rem', 
              background: 'var(--bg-secondary)', 
